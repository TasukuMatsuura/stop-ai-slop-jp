...
15. **総論→各論の順で並べる**。概要が先、詳細が後。マクロからミクロへ。全体像より先に個別の細部が来ていないか確認し、逆や混在なら並べ替える。詳しくは `references/structures.md` の系統7。
16. **初出の用語・略語・固有名詞は、その場で説明する**。読み手は上から1回読む。説明より前にその語を使わない。略語は初出でフル表記、以降は略語で揃える。
17. **単語の羅列で済ませない**。意味の説明を名詞・体言止めの羅列に省略しない。関係・理由・条件は文(主語と述語のある文)で書く。箇条書きは文の代用ではなく、文で書いた内容の整理・一覧に使う。
18. **同じことを各所に散らさない(DRY)**。同一の説明・定義・主張を複数箇所に重複させない。一次情報は1箇所に置き、他からは参照する。局所的な重複は系統3-1・3-4も参照。
...
- [ ] 概要→詳細・総論→各論・マクロ→ミクロの順か?(細部が全体像より先に出ていないか)
- [ ] 初出の専用語・略語・固有名詞を、説明なしでいきなり使っていないか?
- [ ] 単語・体言止めの羅列だけで意味を伝えようとしていないか?
- [ ] 同じ説明・定義・主張を複数箇所に重複させていないか?(DRY)
...
- `references/structures.md` — 避けるべき構造パターン7系統。false agency、必殺技造語、全方位肯定、構成の設計不全など。
